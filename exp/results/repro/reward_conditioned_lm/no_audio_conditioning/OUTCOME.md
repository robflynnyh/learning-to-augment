...
| Reward-control average masks | Averaged `10,000` UVQLM masks plus `10,000` sampled masks at reward `0.0` and `10,000` at reward `1.0` from the 384/dropout checkpoint without storing all masks. The figures plot masked percentage: `0%` means fully retained/unmasked and `100%` means fully suppressed/masked out. Average masked percentage is `49.13%` for UVQLM, `70.18%` at reward `0.0`, and `33.19%` at reward `1.0`. | `visualizations/reward_conditioned_average_masks_10k/metadata.json` |
...
reward conditioning against the UVQLM baseline: UVQLM sits between reward `0.0`
and reward `1.0`, while higher reward shifts the sampled multiplicative masks
