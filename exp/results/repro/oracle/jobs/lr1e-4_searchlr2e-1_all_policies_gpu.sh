...
SCREEN_NAME="${SCREEN_NAME:-l2a_oracle_all_lr1e4_searchlr2e1}"
RUNNER_LABEL="${RUNNER_LABEL:-screen:${SCREEN_NAME}}"
LOG_PATH="${LOG_PATH:-exp/results/repro/oracle/logs/lr1e-4_searchlr2e-1_all_policies_gpu.log}"
RESULTS_PATH="${RESULTS_PATH:-exp/results/repro/oracle}"
QUEUED_COMMAND="${QUEUED_COMMAND:-screen -L -Logfile exp/results/repro/oracle/logs/lr1e-4_searchlr2e-1_all_policies_queue.log -dmS l2a_oracle_all_lr1e4_searchlr2e1 bash -lc 'cd /exp/exp4/acp21rjf/symphony-workspaces-learning-to-augment/ROB-60 && /store/store5/software/simple-gpu-schedule/with-gpu 1,2 -- bash exp/results/repro/oracle/jobs/lr1e-4_searchlr2e-1_all_policies_gpu.sh'}"
...
on_exit() {
  ...
  python3 scripts/callbacks/linear_experiment_callback.py \
    ...
    --branch "${GIT_BRANCH}" \
    --commit "${GIT_COMMIT}" \
    --note "ROB-60 RMM/RFM/UVQLM oracle sweep lr=1e-4/search_lr=2e-1 finished. Inspect text files and update OUTCOME/plot before final handoff." \
    "${callback_args[@]}"
  callback_status=$?
...
echo "[$(date -Iseconds)] ROB-60 RMM/RFM/UVQLM oracle sweep lr=1e-4 search_lr=2e-1"
...
run_method() {
  local method="$1"
  local result="exp/results/repro/oracle/${method}/tedlium_lr1e-4_searchlr2e-1.txt"
  ...
  for repeats in 1 2 3 4 5 10 20 50; do
    ...
    PYTHONDONTWRITEBYTECODE=1 python3 exp/oracle_eval.py \
      --config "${config_dir}/tedlium_lr1e-4_searchlr2e-1_repeats${repeats}.yaml"
  done
}
...
echo "[$(date -Iseconds)] completed ROB-60 RMM/RFM/UVQLM oracle sweep lr=1e-4 search_lr=2e-1"
