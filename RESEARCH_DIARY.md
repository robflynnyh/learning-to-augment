...
- Extended ROB-109 after review with behavior-level VQ-sequence verification:
  ...
  `10.7293`.
- Added the ROB-111 review plan for a no-audio reward-conditioned UVQLM under
  `exp/results/repro/unconditional_lm/ROB-111_reward_conditioned_vq_lm_plan/`.
  The plan recommends training from saved ROB-109 VQ `generation` sequences,
  using global train-split WER-delta normalization, and building a slim
  generation/reward dataset to avoid raw float8 rollout deserialization during
  training.
