...
on_exit() {
  ...
  callback_args=(
    ...
    --note "${CALLBACK_NOTE:-ROB-186 full TED-LIUM test evaluation comparing causal-chunk and stitched-overlap eval for seed ASR, step-0 random-init updater, and latest plasticity checkpoint exited. Inspect ${LOG_PATH}, ${RESULT_ROOT}/summary.json, ${RESULT_ROOT}/per_recording.csv, and ${RESULT_ROOT}/OUTCOME.md before final handoff.}"
...
eval_overrides=(
  ...
  "evaluation.variants=[seed_asr,seed_asr_stitched,step0_random_init,step0_random_init_stitched,latest_checkpoint,latest_checkpoint_stitched]"
