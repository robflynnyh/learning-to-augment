...
RESULT_ROOT="${RESULT_ROOT:-${REPO_DIR}/exp/results/plasticity_eggroll/full_test_seed_vs_latest_$(date -u +%Y%m%dT%H%M%SZ)}"
...
on_exit() {
  ...
  callback_args=(
    ...
    --note "${CALLBACK_NOTE:-ROB-186 full TED-LIUM test evaluation comparing seed ASR against latest plasticity checkpoint exited. Inspect ${LOG_PATH}, ${RESULT_ROOT}/summary.json, ${RESULT_ROOT}/per_recording.csv, and ${RESULT_ROOT}/OUTCOME.md before final handoff.}"
...
eval_overrides=(
  ...
  "evaluation.variants=[seed_asr,latest_checkpoint]"
