#!/usr/bin/env bash
# Callback-backed ROB-132 audio SSL-conditioned fixed-reward self-training eval.
...
DATASET="${ROB132_SELFTRAIN_DATASET:-tedlium}"
SPLIT="${ROB132_SELFTRAIN_SPLIT:-dev}"
REWARDS="${ROB132_SELFTRAIN_REWARDS:-1.0 0.0}"
EPOCHS="${ROB132_SELFTRAIN_EPOCHS:-1 5}"
LR="${ROB132_SELFTRAIN_LR:-1e-5}"
QUEUED_COMMAND="${QUEUED_COMMAND:-screen -L -Logfile ${SCREEN_LOG_PATH} -dmS ${SCREEN_NAME} bash -lc 'cd ${REPO_DIR} && /store/store5/software/simple-gpu-schedule/with-gpu 1,2 -- scripts/launch_rob132_audio_ssl_self_train_fixed_rewards.sh'}"
...
on_exit() {
  status=$?
  set +e
  if [ "${ROB132_SELFTRAIN_DISABLE_CALLBACK:-0}" = "1" ]; then
    exit "${status}"
  fi
  if [ -z "${LINEAR_API_KEY:-}" ] && [ "${ROB132_SELFTRAIN_CALLBACK_DRY_RUN:-0}" != "1" ]; then
  ...
  callback_args=(
    ...
    --note "${CALLBACK_NOTE:-ROB-132 audio SSL fixed-reward self-training eval wrapper exited. Inspect OUTCOME.md under ${RESULT_ROOT}, then post the four-cell result summary.}"
  ...
  if [ "${ROB132_SELFTRAIN_CALLBACK_CHECK_ONLY:-0}" = "1" ]; then
    callback_args+=(--check-only)
  fi
  if [ "${ROB132_SELFTRAIN_CALLBACK_DRY_RUN:-0}" = "1" ]; then
...
echo "[rob132-selftrain] branch=${GIT_BRANCH}"
echo "[rob132-selftrain] commit=${GIT_COMMIT}"
echo "[rob132-selftrain] host=$(hostname)"
echo "[rob132-selftrain] result_root=${RESULT_ROOT}"
echo "[rob132-selftrain] checkpoint=${CHECKPOINT_PATH}"
echo "[rob132-selftrain] asr_ckpt=${ASR_CKPT}"
echo "[rob132-selftrain] mask_vae_ckpt=${MASK_VAE_CKPT}"
echo "[rob132-selftrain] dataset=${DATASET}"
echo "[rob132-selftrain] split=${SPLIT}"
echo "[rob132-selftrain] rewards=${REWARDS}"
echo "[rob132-selftrain] epochs=${EPOCHS}"
echo "[rob132-selftrain] lr=${LR}"
echo "[rob132-selftrain] log_path=${LOG_PATH}"
echo "[rob132-selftrain] screen_log_path=${SCREEN_LOG_PATH}"
echo "[rob132-selftrain] queued_command=${QUEUED_COMMAND}"
echo "[rob132-selftrain] cuda_visible_devices=${CUDA_VISIBLE_DEVICES:-unset}"
echo "[rob132-selftrain] tmpdir=${TMPDIR}"
echo "[rob132-selftrain] torch_home=${TORCH_HOME}"
...
print("[rob132-selftrain] python_executable=" + sys.executable)
print("[rob132-selftrain] python_version=" + sys.version.split()[0])
print("[rob132-selftrain] torch_version=" + torch.__version__)
print("[rob132-selftrain] torchaudio_version=" + torchaudio.__version__)
print("[rob132-selftrain] cuda_available=" + str(torch.cuda.is_available()))
if torch.cuda.is_available():
    print("[rob132-selftrain] cuda_device=" + torch.cuda.get_device_name(0))
PY'

if [ "${ROB132_SELFTRAIN_CALLBACK_ONLY:-0}" = "1" ]; then
  echo "[rob132-selftrain] callback-only smoke path requested; exiting before config generation."
  exit 0
fi
...
if [ ! -d "${L2A_TEDLIUM3_LEGACY_DIR}" ]; then
  echo "Missing TED-LIUM directory: ${L2A_TEDLIUM3_LEGACY_DIR}" >&2
  exit 1
fi

python3 - "${RESULT_ROOT}" "${ASR_CKPT}" "${CHECKPOINT_PATH}" "${MASK_VAE_CKPT}" "${LINEAR_ISSUE}" "${DATASET}" "${SPLIT}" "${REWARDS}" "${EPOCHS}" "${LR}" <<'PY'
import sys
...
dataset = sys.argv[6]
split = sys.argv[7]
rewards = tuple(float(item) for item in sys.argv[8].split())
epochs = tuple(int(item) for item in sys.argv[9].split())
lr = sys.argv[10]

def reward_tag(value):
...
evaluation:
  id: {linear_issue}-{dataset}-{split}-audio-ssl-transformer384-reward{reward_tag(reward)}-epoch{epoch_count}-lr{lr}
  dataset: {dataset}
  split: {split}
...
""",
        ...
        print(f"[rob132-selftrain] wrote config {config_path}")
PY

python3 scripts/summarize_rob132_audio_ssl_self_train_fixed_rewards.py \
...
if [ "${ROB132_SELFTRAIN_CONFIG_ONLY:-0}" = "1" ]; then
  echo "[rob132-selftrain] config-only smoke path requested; exiting before eval."
  exit 0
fi

RUN_REWARDS="${ROB132_SELFTRAIN_RUN_REWARDS:-${REWARDS}}"
RUN_EPOCHS="${ROB132_SELFTRAIN_RUN_EPOCHS:-${EPOCHS}}"
EVAL_SCRIPT="${ROB132_SELFTRAIN_EVAL_SCRIPT:-eval.py}"

if [ "${ROB132_SELFTRAIN_SMOKE:-0}" = "1" ]; then
  RUN_REWARDS="${ROB132_SELFTRAIN_SMOKE_REWARDS:-1.0}"
  RUN_EPOCHS="${ROB132_SELFTRAIN_SMOKE_EPOCHS:-1}"
  ROB132_SELFTRAIN_INDEXES="${ROB132_SELFTRAIN_INDEXES:-0}"
  ROB132_SELFTRAIN_DONT_SAVE="${ROB132_SELFTRAIN_DONT_SAVE:-1}"
  echo "[rob132-selftrain] smoke mode: rewards=${RUN_REWARDS}; epochs=${RUN_EPOCHS}; indexes=${ROB132_SELFTRAIN_INDEXES}; dont_save=${ROB132_SELFTRAIN_DONT_SAVE}"
fi
...
for reward in ${RUN_REWARDS}; do
  ...
  for epoch_count in ${RUN_EPOCHS}; do
    tag="${DATASET}_${SPLIT}_reward${reward_tag}_epoch${epoch_count}_lr${LR}"
    ...
    if [ "${FORCE_RERUN:-0}" != "1" ] && [ -f "${save_path}" ] && grep -q "Updated_WER:" "${save_path}"; then
      echo "[rob132-selftrain] skipping completed reward=${reward}/epoch${epoch_count}: ${save_path}"
      continue
    fi
    ...
    CONFIG_TO_RUN="${config}" EVAL_SCRIPT="${EVAL_SCRIPT}" bash -ic '
      ...
      if [ -n "${ROB132_SELFTRAIN_INDEXES:-}" ]; then
        read -r -a indexes <<< "$ROB132_SELFTRAIN_INDEXES"
        args+=(--indexes "${indexes[@]}")
      fi
      if [ "${ROB132_SELFTRAIN_DONT_SAVE:-0}" = "1" ]; then
        args+=(--dont_save)
      fi
      echo "[rob132-selftrain] eval command: ${args[*]}"
      "${args[@]}"
    '
  done
done
...
echo "[rob132-selftrain] finished"
