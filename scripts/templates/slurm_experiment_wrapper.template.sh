#!/usr/bin/env bash
# Template for Learn-to-Augment long-running experiments submitted on Stanage with Slurm.
...
REPO_DIR="${REPO_DIR:-/mnt/parscratch/users/acp21rjf/symphony-workspaces-learning-to-augment/ROB-000}"
LINEAR_ISSUE="${LINEAR_ISSUE:-ROB-000}"
RESULTS_PATH="${RESULTS_PATH:-results/example}"
LOG_PATH="${LOG_PATH:-/mnt/parscratch/users/acp21rjf/symphony-job-artifacts/l2a-example-${SLURM_JOB_ID:-manual}.log}"
...
python3 -m compileall l2augment exp 2>&1 | tee -a "${LOG_PATH}"
